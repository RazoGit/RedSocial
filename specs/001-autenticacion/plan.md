...
| D8  | Rate limit con **@nestjs/throttler + storage Redis** _(enmienda T8: servicio propio `LoginRateLimiterService` sobre ioredis)_             | En memoria                         | Necesario para múltiples instancias ya. El throttler cuenta peticiones, pero RF-5 exige contar solo fallos y resetear el contador al acertar; un servicio dedicado implementa la semántica exacta en ~100 líneas sin dependencia extra |
| D9  | Validación de DTOs con **Zod de `@redsocial/contracts`** (pipe propio) en vez de class-validator                                          | class-validator + ValidationPipe   | Constitución principio 3: esquemas compartidos front/back; Zod 4 genera el JSON Schema para Swagger sin dependencias extra (decisión del propietario, 2026-08-22)                                                                      |
...
├─ dto/                      # esquemas Zod de @redsocial/contracts + JSON schemas para Swagger
